...
- 流量强度$=\dfrac{分组长度\times分组平均到达速率}{链路带宽}$

  流量强度$\rightarrow 0$：**平均**排队延迟**很小**。

  流量强度$\rightarrow 1$：**平均**排队延迟**很大**。
...
H1发送整个文件的传输延迟为
$$
\dfrac{(980000+20\times 1000)\times 8}{100000000}=80{\rm ms}
$$
最后一个分组的两次转发传输延迟为
$$
\dfrac{2\times 1000\times 8}{100000000}=0.16{\rm ms}
$$
总用时为$80+0.16=80.16{\rm ms}$。
