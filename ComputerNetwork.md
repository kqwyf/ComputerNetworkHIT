...
- **循环冗余校验码**（CRC）：将数据看作二进制数D，并选择一个r+1位的**生成比特模式**G，选择r位的CRC比特R，满足在模2意义下<D,R>恰好可被G整除。接收端只需判断G除以<D,R>，若余式全0则无错，否则有错。CRC可检测**所有突发长度小于r+1位的差错**。
...
- **信道划分MAC协议**：采用**多路复用技术**划分信道，如TDMA、FDMA、CDMA、WDMA等（参见*第0周：多路复用的方法*）。
  ...
  - TDMA：每个节点在每个周期中占用**固定长度的时隙**，未被节点占用的时隙**空闲**。
  - FDMA：每个节点占用一个**固定频带**，未被节点占用的频带**空闲**。

- **随机访问MAC协议**（多见于**局域网**）：不划分信道，节点使用**最大速率**发送分组。**允许冲突**，采用**冲突恢复机制**。如时隙ALOHA、纯ALOHA、CSMA等。

  - 时隙ALOHA协议

    假设：帧大小相同、时间划分为等长时隙，且每个时隙可传输1帧，帧只能在时隙开始时刻发送。节点间时钟同步。

    协议内容：节点需发送帧时，**等待下一个时隙并发送**。若冲突，节点在下一个时隙**以概率p重传**，直至成功。

    优点：单个节点可使用全部带宽，节点高度分散化。

    缺点：容易造成冲突，容易造成空闲时隙；节点可能能以远小于时隙的时间检测冲突；要求时钟同步。

    **效率**：长期运行该协议时，成功发送帧的时隙所占比例。时隙ALOHA协议在节点数量极大时，**效率关于p的最大值为1/e≈0.37**。
  ...
  - 纯ALOHA协议
    ...
    **效率**：计算方法同上，**效率关于p的最大值为1/(2e)≈0.18**。

  - 载波监听多路访问（CSMA）协议

    协议内容：节点需发送帧时，**监听信道**。若信道空闲，则发送完整帧；若信道忙，则推迟一段时间后以概率p发送（根据p的取值可分为非坚持、p坚持与1坚持）。

    缺点：由于信号传播延迟，冲突仍可能发生。

    **以太网**使用**CSMA/CD协议**，**802.11无线局域网**使用**CSMA/CA协议**。

    - CSMA/CD协议：发送的同时检测冲突，若发生冲突则**中止传输**。

      **有效条件**（冲突可检测到的条件）：**L/R≥2d/V**，其中L为数据帧最小长度，R为带宽，d为网络跨距（最远节点距离），V为信号传播速度。对于一般网络应有**L/R≥RTT**。

      缺点：冲突检测在有线局域网中易于实现，但无线局域网难以实现。

      **效率**：经计算得效率为**1/(1+5T/t)**，其中T为两节点间最大传播延迟，t为最长帧传输延迟。

- **轮转MAC协议**：节点**轮流**使用信道的**全部带宽**。

  **蓝牙**使用**轮转MAC协议**。

  - 轮询协议

    协议内容：主节点**轮流邀请**从属节点发送数据。

    缺点：轮询存在开销、等待延迟；存在单点故障问题。

  - 令牌传递协议

    协议内容：**控制令牌**（特殊帧）依次从一个节点传递到下一个节点，仅拥有令牌的节点可以发送数据。令牌网络通常是**环形网络**。

    缺点：传输令牌存在开销、等待延迟；存在单点故障问题（令牌丢失）。

### 地址解析协议（ARP）

**MAC地址**（又称LAN地址、物理地址、以太网地址）：

- 在**数据链路层**标识物理相连的**接口**，通常为48位，以6组2位16进制数书写，以短横隔开，如1A-2F-BB-76-09-AD。
- **局域网**中的**每块网卡**都有一个**唯一的MAC地址**。
- MAC地址由IEEE统一管理与分配，网卡生产商可购买MAC地址空间。

**ARP表**：局域网中每个IP节点维护一个ARP表，维护IP/MAC映射关系（IP；MAC；TTL）。设置TTL是由于IP地址可能被更换，映射可能失效。

ARP功能：在同一个局域网内，已知目的接口IP地址时，获取其MAC地址。

##### 协议内容

- 发送方**广播**（目的MAC地址为FF-FF-FF-FF-FF-FF）**ARP查询分组**，包含接收方**IP地址**。
- 接收方收到ARP查询分组并发现**IP地址与自身匹配**，向发送方**单播**（目的MAC地址为发送方）**ARP响应分组**。
- 发送方收到ARP响应分组后在ARP表中**缓存**接收方的IP-MAC地址对。

ARP是**即插即用**协议。

**跨局域网路由**时，**路由器**不改变IP数据报的源IP与目的IP，但会**修改源MAC地址与目的MAC地址**。

### 以太网

服务类型：无连接、不可靠（误码率小，差错帧直接**丢弃**）服务。

不同以太网标准**MAC协议**与**帧格式相同**，而**速率及物理介质等不同**。

#### 物理拓扑

- 总线（也称**广播以太网**，上世纪90年代中期流行）：所有节点在同一**冲突域**，即任一节点发送数据均有可能与其他连接在同一总线上的节点发生冲突。
- 星形（也称**交换以太网**，当前的主流网络拓扑）：以**交换机**作为中心设备，每个节点在一个**单独冲突域**中。

#### MAC协议

协议为**使用二进制退避算法的CSMA/CD**，在**网卡**中实现。
...
- 发送过程中检测是否有其它节点的数据发送：
  - 若未发现则**确认**该帧发送成功。
  - 若发现则**中止发送**并发送**堵塞信号**（便于网络中其它节点尽快发现冲突），进入**二进制指数退避**：
    - 在第m次连续冲突后，取n=max(m,10)。
    - 在0~2^n-1中随机选择数K，并等待512K bit的传输延迟时间后，重新尝试发送。
    - （以太网中）连续冲突16次后不再尝试并向上层协议报告错误。

#### 帧结构

| 字段        | 长度        | 说明                                                         |
| ----------- | ----------- | ------------------------------------------------------------ |
| 前导码      | 8字节       | 前7字节均为0xAA，第8字节为0xAB。用于发送/接收端时钟同步。通常不计入帧长度。 |
| 目的MAC地址 | 6字节       | 说明目的MAC地址。                                            |
| 源MAC地址   | 6字节       | 说明源MAC地址。                                              |
| 类型        | 2字节       | 说明上层协议类型。                                           |
| 数据        | 46~1500字节 | 数据载荷。                                                   |
| CRC         | 4字节       | 循环冗余校验码。                                             |
...
功能：**存储转发**以太网帧。即检验到达帧**目的MAC地址**并**选择性**地向某些输出链路转发帧，当转发方向不同时可**并行**进行。使用CSMA/CD协议。

**透明交换机**（最典型、最广泛）：主机感知不到交换机的存在，**即插即用**，**自学习**（无需配置）。

**交换表**：记录三元组**(主机MAC地址,到达主机的接口,时间戳)**，其中时间戳用于使记录失效。

交换机可以互联，**工作方式不变**。

##### 工作方式

- 自学习：收到帧时，记录**发送主机**的**MAC地址**及**接口**。

- 帧过滤/转发：若目的接口与接收接口相同，则**丢弃帧**；若未学习到目的接口，则向所有接口转发（**泛洪**）。

由于同一IP子网中广播帧将被交换机泛洪至所有主机，故IP子网又称为**广播域**。
...
- 基于端口：将交换机端口分组以**隔离流量**，一组内的端口通信不影响另一组端口。也可基于MAC地址分组。
  - **动态成员**：端口可**动态分配**给不同VLAN。
  - VLAN间转发：通过**路由器**转发（在交换机两组端口间连接路由器，实践中路由器常集成在交换机上）。
  - 跨交换机VLAN：在交换机上设置**中继端口**，可在两交换机间传输所有VLAN的帧，使用802.1Q协议标记VLAN号。

### 点对点协议（PPP）

优势：无需介质访问控制、无需明确MAC寻址。

功能：组帧（能承载任何网络层协议分组，实现分用），比特透明传输（数据域支持承载任何比特模式），差错检测，连接活性检测（并向上层通报错误），网络层地址协商（端节点可学习/配置彼此网络地址）。

#### 协商内容

- 配置PPP链路：最大帧长、身份认证（可选）等。
- 配置网络层信息：（对于IP协议）交换IPCP报文以完成配置。

#### 帧结构

| 字段 | 长度     | 说明                                                         |
| ---- | -------- | ------------------------------------------------------------ |
| 标志 | 1字节    | 值为01111110，用于标记数据帧边界。                           |
| 地址 | 1字节    | 值为全1，无意义。可被协商省略。                              |
| 控制 | 1字节    | 无意义。可被协商省略。                                       |
| 协议 | 1或2字节 | 说明上层协议类型。字段长度通过协商确定。                     |
| 信息 | 可变     | 上层协议分组数据。当数据中出现0x7E（标志字段）或0x7D时，在其前插入0x7D作为转义符。 |
| 校验 | 2或4字节 | CRC校验码，用于差错检测。                                    |
| 标志 | 1字节    | 值为01111110，用于标记数据帧边界。（与首字段相同）           |

### 802.11无线局域网

**基本服务集**（单元）：基站覆盖的主机范围。自组网模式中所有主机构成一个单元。

#### 常见版本

所有版本均有**基础设施（基站）网络模式**和**特定网（自组网）网络模式**。

| 标准    | 频段         | 最大数据速率 | 物理层    | 优缺点                                                     |
| ------- | ------------ | ------------ | --------- | ---------------------------------------------------------- |
| 802.11b | 2.4GHz       | 11Mbps       | 扩频      | 最高数据率较低，价格最低，信号传播距离最远。               |
| 802.11a | 5GHz         | 54Mbps       | OFDM      | 最高数据率较高，支持更多用户，价格最高，信号传播距离较短。 |
| 802.11g | 2.4GHz       | 54Mbps       | OFDM      | 最高数据率较高，支持更多用户，信号传播距离最远。           |
| 802.11n | 2.4GHz、5GHz | 600Mbps      | MIMO OFDM | 使用多个发射和接收天线达到更高数据传输率。                 |

#### 信道与基站关联（以802.11b为例）

每个基站在频段的11个信道中选择一个信道，选择相同或相邻信道的两基站**可能存在干扰**。

#### 主机与基站关联（以802.11b为例）

- 被动扫描：基站发送**信标帧**，主机向选择的基站发送**关联请求帧**，基站向主机发送**关联响应帧**。
- 主动扫描：主机**广播探测请求帧**，基站发送**探测响应帧**，主机向选择的基站发送**关联请求帧**，基站向主机发送**关联响应帧**。

#### 多路访问控制

802.11所有版本均使用**CSMA/CA**多路访问控制协议，由于无线信号**不支持边发送边检测冲突**，且**无法侦听到所有可能冲突**。

##### 协议内容

- 发送方：检测到**信道空闲DIFS时间**后才发送整个帧；若检测到信道忙，进行随机退避计时，当**计时超时**且**信道空闲DIFS时间**后发送整个帧。

  发送后等待ACK帧，若未收到，增加随机退避间隔时间并重发。

- 接收方：正确接收帧后，**延迟SIFS时间**后发送ACK帧。（SIFS<DIFS）

- 冲突避免：发送端预约信道以避免数据帧冲突。

  - 发送端开始发送帧时在**信道空闲DIFS时间**后发送**RTS帧**，由于RTS帧很短，故**冲突损失小**。
  - 基站收到RTS帧后**等待SIFS时间**并**广播CTS帧**作为响应。其余节点收到CTS后**不再征用信道**。
  - 发送端收到CTS后**等待SIFS时间**开始**发送数据帧**。
  - 基站接收完毕后**等待SIFS时间**并**广播ACK帧**。

##### 帧结构（部分）

| 字段         | 长度  | 说明                                                     |
| ------------ | ----- | -------------------------------------------------------- |
| 协议版本号   | 2位   | 802.11协议版本号。                                       |
| 帧类型       | 2位   | 说明帧类型，如RTS、CTS、ACK、Data。                      |
| 子类型       | 4位   | 与上一字段共同表示帧类型。                               |
| 去往基站     | 1位   | 说明该帧是否去往基站。                                   |
| 来自基站     | 1位   | 说明该帧是否来自基站。                                   |
| 更多分片     | 1位   | 说明是否还有后续分片。                                   |
| 预约传输时间 | 2字节 | 说明信道占用时间，单位为微秒。                           |
| 地址1        | 6字节 | 在去往基站帧中表示基站地址，在来自基站帧中表示目的地址。 |
| 地址2        | 6字节 | 在去往基站帧中表示源地址，在来自基站帧中表示基站地址。   |
| 地址3        | 6字节 | 在去往基站帧中表示目的地址，在来自基站帧中表示源地址。   |
| 帧序号       | 2字节 | 用于可靠数据传输。                                       |
| 地址4        | 6字节 | 用于自组网络。                                           |
...
由于L/R=2d/V，有d=(VL)/(2R)，则△d=(V△L)/(2R)。
